<div class="container-fluid">
    <!-- *************************************************************** -->
    <!-- Start First Cards -->
    <!-- *************************************************************** -->
    <div class="card-group">
        <div class="col-md-6">
            <div class="card border-info">
                <div class="card-header bg-info">
                    <h3 class="mb-0 text-white">HÓA ĐƠN</h3>
                </div>
                <div class="card-body">
                    <!-- Từ trung tâm -->
                    <p class="card-text">Từ: </p>
                    <p class="card-text">Trung tâm gia sư Wibu</p>
                    <p class="card-text">contact@wibuteam.phatdev.xyz</p>
                    <p class="card-text">VQ4P+249, Phường Tân Phú, Quận 9, Thành phố Hồ Chí Minh</p>
                    <p class="card-text">0358125455</p>
                    <hr width="90%" color="gray" align="center"">
                                <!-- Đến học sinh -->
                                <p class=" card-text">Đến: </p>
                    <p class="card-text">Ngô Thế Bách</p>
                    <p class="card-text">ngothebach181001@gmail.com</p>
                    <p class="card-text">Quận 9, Thành phố Hồ Chí Minh</p>
                    <p class="card-text">SĐT: 0358125655</p>
                    <hr width="90%" color="gray" align="center"">
                                <h4 class=" card-title">Khóa học ôn thi lớp 9</h4>
                    <p class="card-text">Giá tiền: 500.000.000đ </p>
                    <button type="button" class="btn waves-effect waves-light btn-rounded btn-danger">HỦY</button>
                    <button type="button" class="btn waves-effect waves-light btn-rounded btn-success">DUYỆT</button>
                </div>
            </div>
        </div>
    </div>
    <!-- *************************************************************** -->
    <!-- End First Cards -->
    <!-- *************************************************************** -->

    <!-- *************************************************************** -->
</div>